...
function GreyHandling.functions.DisplayMutuallyBeneficialTradeInChat(exchange)
	...
	if exchange.lossCountGiven ~= 0 then
		msg = msg.."|cff"..GreyHandling.redPrint..GreyHandling[" (their stack overflow by "]..exchange.lossCountGiven..")|r "
	end
	...
	if exchange.lossCountTaken ~= 0 then
		msg = msg.."|cff"..GreyHandling.redPrint..GreyHandling[" (your stack overflow by "]..exchange.lossCountTaken..")|r "
	end
	if exchange.ourGain < 0 then
		msg = format(GreyHandling["%s and %s"], msg, GetCoinTextureString(-exchange.ourGain))
	end
	return msg
end
